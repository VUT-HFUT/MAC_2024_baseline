...
class MMAD(torch.utils.data.Dataset):
    def __init__(self, anno_file, frame_file, frame_folder, split, window_size, interval, num_classes, img_tensor, tensor_folder):
        ...
        overlap_dict = {'train': 4, 'test': 1, 'val': 1}
        ...
        for vid in video_list:
            if self.anno_file[vid]['subset'] == self.split:
                ...
                if seq_len <= self.window_size:
                    ...
                    if self.split=='train' and len(gt) == 0:
                        print(vid)
...
def build_mmad(split, args):
    return MMAD(args.annotation_path, args.frame_file_path, args.frame_folder, split, args.window_size, args.interval, args.num_classes, args.img_tensor, args.tensor_folder)
